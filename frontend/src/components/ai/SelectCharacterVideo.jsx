...
export default function SelectCharacterVideo() {
  ...
  useEffect(() => {
    ...
    const fetchChars = async () => {
      ...
      try {
        ...
        setCharacters(mapped.length ? mapped : []);
      } catch (err) {
  ...
  const onSelect = (character) => {
    try { localStorage.setItem("pronily:video:selectedCharacter", JSON.stringify(character)); } catch (e) {}
  // return to video generator so it picks up the selection
